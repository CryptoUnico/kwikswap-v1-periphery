...
import '@kwikswap/v1-core/contracts/interfaces/IKwikswapV1Callee.sol';

import '../libraries/KwikswapV1Library.sol';
...
contract ExampleFlashSwap is IKwikswapV1Callee {
    ...
    constructor(address _factory, address _factoryV, address router) public {
        ...
        factory = _factory;
        WETH = IWETH(IKwikswapV1Router01(router).WETH());
    }
    ...
    function kwikswapV1Call(address sender, uint amount0, uint amount1, bytes calldata data) external override {
        address[] memory path = new address[](2);
        uint amountToken;
        uint amountETH;
        { // scope for token{0,1}, avoids stack too deep errors
        address token0 = IKwikswapV1Pair(msg.sender).token0();
        address token1 = IKwikswapV1Pair(msg.sender).token1();
        assert(msg.sender == KwikswapV1Library.pairFor(factory, token0, token1)); // ensure that msg.sender is actually a V1 pair
        ...
        IKwikswapVExchange exchangeV = IKwikswapVExchange(factoryV.getExchange(address(token))); // get V exchange
        ...
        if (amountToken > 0) {
            ...
            uint amountRequired = KwikswapV1Library.getAmountsIn(factory, amountToken, path)[0];
            ...
            uint amountRequired = KwikswapV1Library.getAmountsIn(factory, amountETH, path)[0];
